...
/**
 * Multi-note editor for a checklist item or subtask.
 * Loads notes for this item plus shared notes from sibling items
 * (same checklist_items.template_id across other production lines).
 */
export function ItemNotesEditor({
  ...
  const load = async () => {
    ...
    if (itemTemplateId) {
      const { data: sibs } = await supabase
        .from("checklist_items").select("id")
        .eq("template_id", itemTemplateId).neq("id", itemId).is("deleted_at", null);
      siblingIds = (sibs ?? []).map((s: any) => s.id);
    }
    const orFilter = siblingIds.length > 0
      ? `item_id.eq.${itemId},and(is_shared.eq.true,item_id.in.(${siblingIds.join(",")}))`
      : `item_id.eq.${itemId}`;
    const { data } = await supabase
      .from("item_notes" as any).select("*")
      .or(orFilter)
  ...
  const update = (id: string, patch: Partial<ItemNote>) => {
    ...
    supabase.from("item_notes" as any).update(patch as any).eq("id", id).then();
  };

  const remove = async (n: ItemNote) => {
    undoableDelete({
      label: "Note deleted",
      ...
      commit: async () => {
        const [{ data: photos }, { data: files }] = await Promise.all([
          supabase.from("note_photos" as any).select("storage_path").eq("parent_kind", "item_note").eq("parent_id", n.id),
          supabase.from("note_files" as any).select("storage_path").eq("parent_kind", "item_note").eq("parent_id", n.id),
        ]);
        ...
        await supabase.from("note_photos" as any).delete().eq("parent_kind", "item_note").eq("parent_id", n.id);
        await supabase.from("note_files" as any).delete().eq("parent_kind", "item_note").eq("parent_id", n.id);
        await supabase.from("item_notes" as any).delete().eq("id", n.id);
      },
      afterCommit: load,
    });
  };
      ...
      {open && (
        <div className="space-y-2 p-2">
          <Textarea value={body} disabled={!canEdit}
            ...
            data-resize-key={`item-note:${note.id}`}
            className="min-h-[50px] resize-y text-xs" />
          <NoteAttachments
            parentKind="item_note"
            parentId={note.id}
            storagePrefix={`item-notes/${note.item_id}/${note.id}`}
            canEdit={canEdit}
            userId={userId}
