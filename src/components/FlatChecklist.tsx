          ...
          <SortableContext items={rootItems.map((i: any) => i.id)} strategy={verticalListSortingStrategy}>
            <ul className="space-y-1">
              {rootItems.map((it: any) => (
                <TreeNode key={it.id} item={it} allItems={allItems} canEdit={canEdit} onChange={onChange} depth={0} sortable />
              ))}
            </ul>
          </SortableContext>
        </DndContext>
...
function PhotoTile({ path, canEdit, onRemove }: { path: string; canEdit: boolean; onRemove: () => void }) {
...
function FileChip({ f, canEdit, onRemove }: { f: any; canEdit: boolean; onRemove: () => void }) {
  const open = async () => {
